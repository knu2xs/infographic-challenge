...
- `make env` - builds the Conda environment with all the name and dependencies from `environment_dev.yml` and installs the local project package `infographic_challenge` using the command `python -m pip install -e ./src/src/infographic_challenge` so you can easily test against the package as you are developing it.
